...
<body>
  ...
  <div class="title-section">
    ...
    <p>Periode Data: <?= format_indo($awal) ?> s/d <?= format_indo($akhir) ?></p>
  </div>
  ...
  <table class="data-table">
    <thead>
      <tr>
        ...
        <th class="text-left" style="width: 40%;">Nama Barang</th>
    ...
    <tbody>
      ...
      $total_stok = 0;
      foreach ($okta as $gas) {
          $total_stok += $gas->stok_product;
      ?>
        <tr>
          <td class="text-center"><?= $no++ ?></td>
          <td class="text-left"><?= esc($gas->nama_product) ?></td>
          <td class="text-center"><?= esc($gas->kode_product) ?></td>
          <td class="text-center"><?= $gas->stok_product ?> unit</td>
          <td class="text-right">Rp <?= number_format($gas->harga_product, 0, ',', '.') ?></td>
        </tr>
      <?php } ?>
    </tbody>
    <tfoot>
      <tr class="total-row">
        <td colspan="3" class="text-right">Total Seluruh Stok:</td>
        <td class="text-center"><?= $total_stok ?> unit</td>
        <td></td>
      </tr>
    </tfoot>
  </table>
